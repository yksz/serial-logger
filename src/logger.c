...
static long vflog(FILE* fp, const char* timestamp,
        ...
        long currentTime, long* flushedTime)
{
    ...
    if ((size = fprintf(fp, "%s: ", timestamp)) > 0) {
        totalsize += size;
    }
...
void logger_log(const char* fmt, ...)
{
    ...
    if (hasFlag(s_logger, kConsoleLogger)) {
        va_start(carg, fmt);
        vflog(s_clog.output, timestamp,
                fmt, carg, currentTime, &s_clog.flushedTime);
        va_end(carg);
    }
    if (hasFlag(s_logger, kFileLogger)) {
        if (rotateLogFiles()) {
            va_start(farg, fmt);
            s_flog.currentFileSize += vflog(s_flog.output, timestamp,
